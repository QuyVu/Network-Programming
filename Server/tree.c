...
static void mk_new_int(Tree l, Tree r, Tree p, int il);
static Tree lprev(Tree n);
static Tree rprev(Tree n);
static void recolor(Tree n);
static void single_rotate(Tree y, int l);
static void tree_print(Tree t, int level);
static void tree_iprint(Tree t, int level);
...
#define getlext(n) ((struct tree_node *)(n->key.v))
#define setlext(node, val) node->key.v = (void *) (val)
#define getrext(n) ((struct tree_node *)(n->val.v))
...
static void insert(Tree item, Tree list) /* Inserts to the end of a list */ {
    Tree last_node;
...
static void delete_item(Tree item) /* Deletes an arbitrary iterm */ {
    item->flink->blink = item->blink;
    item->blink->flink = item->flink;
}

#define mk_new_ext(new, kkkey, vvval) {\
  new = (Tree) malloc(sizeof(struct tree_node));\
  new->val = vvval;\
  new->key = kkkey;\
  setext(new);\
  setblack(new);\
...
static void mk_new_int(Tree l, Tree r, Tree p, int il) {
    ...
    newnode = (Tree) malloc(sizeof (struct tree_node));
    setint(newnode);
    setred(newnode);
    setnormal(newnode);
    newnode->flink = l;
...
Tree tree_find_gte_str(Tree n, char *key, int *fnd) {
    int cmp;

    *fnd = 0;
    if (!ishead(n)) {
        fprintf(stderr, "tree_find_gte_str called on non-head 0x%lx\n", (intptr_t) n);
        exit(1);
    }
...
Tree tree_find_str(Tree n, char *key) {
    int fnd;
    Tree tree;
    tree = tree_find_gte_str(n, key, &fnd);
    if (fnd) return tree;
    else return NULL;
}

Tree tree_find_gte_int(Tree n, int ikey, int *fnd) {
    *fnd = 0;
    if (!ishead(n)) {
        fprintf(stderr, "tree_find_gte_int called on non-head 0x%lx\n", (intptr_t) n);
        exit(1);
    }
    if (n->parent == n) return n;
...
Tree tree_find_int(Tree n, int ikey) {
    int fnd;
    Tree tree;

    tree = tree_find_gte_int(n, ikey, &fnd);
    if (fnd) return tree;
    else return NULL;
}

Tree tree_find_gte_dbl(Tree n, double dkey, int *fnd) {
    *fnd = 0;
    if (!ishead(n)) {
        fprintf(stderr, "tree_find_gte_dbl called on non-head 0x%lx\n", (intptr_t) n);
        exit(1);
    }
    if (n->parent == n) return n;
...
Tree tree_find_dbl(Tree n, double dkey) {
    int fnd;
    Tree tree;

    tree = tree_find_gte_dbl(n, dkey, &fnd);
    if (fnd) return tree;
    else return NULL;
}

Tree tree_find_gte_gen(Tree n, Value key, int (*fxn)(Value, Value), int *fnd) {
    int cmp;
...
Tree tree_find_gen(Tree n, Value key, int (*fxn)(Value, Value)) {
    int fnd;
    Tree tree;

    tree = tree_find_gte_gen(n, key, fxn, &fnd);
    if (fnd) return tree;
    else return NULL;
}

static Tree tree_insert_b(Tree n, Value key, Value val) {
    Tree newleft, newright, newnode, list, p;
...
void tree_delete_node(Tree n) {
    ...
    if (ishead(n)) {
        fprintf(stderr, "Cannot delete the head of an tree_tree: 0x%lx\n", (intptr_t) n);
        exit(1);
    }
...
void tree_print(Tree t, int level) {
    int i;
    if (ishead(t) && t->parent == t) {
        printf("tree 0x%lx is empty\n", (intptr_t) t);
    } else if (ishead(t)) {
        printf("Head: 0x%lx.  Root = 0x%lx\n", (intptr_t) t, (intptr_t) t->parent);
        tree_print(t->parent, 0);
    } else {
        if (isext(t)) {
            for (i = 0; i < level; i++) putchar(' ');
            printf("Ext node 0x%lx: %c,%c: p=0x%lx, k=%s\n",
                    (intptr_t) t, isred(t) ? 'R' : 'B', isleft(t) ? 'l' : 'r', (intptr_t) t->parent, t->key.s);
        } else {
            tree_print(t->flink, level + 2);
            tree_print(t->blink, level + 2);
            for (i = 0; i < level; i++) putchar(' ');
            printf("Int node 0x%lx: %c,%c: l=0x%lx, r=0x%lx, p=0x%lx, lr=(%s,%s)\n",
                    (intptr_t) t, isred(t) ? 'R' : 'B', isleft(t) ? 'l' : 'r', (intptr_t) t->flink,
                    (intptr_t) t->blink,
                    (intptr_t) t->parent, getlext(t)->key.s, getrext(t)->key.s);
        }
    }
}

void tree_iprint(Tree t, int level) {
    int i;
    if (ishead(t) && t->parent == t) {
        printf("tree 0x%lx is empty\n", (intptr_t) t);
    } else if (ishead(t)) {
        printf("Head: 0x%lx.  Root = 0x%lx, < = 0x%lx, > = 0x%lx\n",
                (intptr_t) t, (intptr_t) t->parent, (intptr_t) t->blink, (intptr_t) t->flink);
        tree_iprint(t->parent, 0);
    } else {
        if (isext(t)) {
            for (i = 0; i < level; i++) putchar(' ');
            printf("Ext node 0x%lx: %c,%c: p=0x%lx, <=0x%lx, >=0x%lx k=%d\n",
                    (intptr_t) t, isred(t) ? 'R' : 'B', isleft(t) ? 'l' : 'r', (intptr_t) t->parent,
                    (intptr_t) t->blink, (intptr_t) t->flink, t->key.i);
        } else {
            tree_iprint(t->flink, level + 2);
            tree_iprint(t->blink, level + 2);
            for (i = 0; i < level; i++) putchar(' ');
            printf("Int node 0x%lx: %c,%c: l=0x%lx, r=0x%lx, p=0x%lx, lr=(%d,%d)\n",
                    (intptr_t) t, isred(t) ? 'R' : 'B', isleft(t) ? 'l' : 'r', (intptr_t) t->flink,
                    (intptr_t) t->blink,
                    (intptr_t) t->parent, getlext(t)->key.i, getrext(t)->key.i);
        }
    }
}

int tree_nblack(Tree n) {
    int nb;
    if (ishead(n) || isint(n)) {
        fprintf(stderr, "ERROR: tree_nblack called on a non-external node 0x%lx\n",
                (intptr_t) n);
        exit(1);
    }
    nb = 0;
...
int tree_plength(Tree n) {
    int pl;
    if (ishead(n) || isint(n)) {
        fprintf(stderr, "ERROR: tree_plength called on a non-external node 0x%lx\n",
                (intptr_t) n);
        exit(1);
    }
    pl = 0;
...
void tree_free(Tree n) {
    if (!ishead(n)) {
        fprintf(stderr, "ERROR: Rb_free_tree called on a non-head node\n");
        exit(1);
    }

    while (TREE_FIRST(n) != TREE_NIL(n)) {
        tree_delete_node(TREE_FIRST(n));
    }
    free(n);
}

Value tree_val(Tree n) {
    return n->val;
}

static Tree tree_insert_a(Tree nd, Value key, Value val) {
    return tree_insert_b(nd->flink, key, val);
}
...
Tree tree_insert_str(Tree tree, char *key, Value val) {
    ...
    k.s = key;
    return tree_insert_b(tree_find_gte_str(tree, key, &fnd), k, val);
}
...
Tree tree_insert_int(Tree tree, int ikey, Value val) {
    ...
    k.i = ikey;
    return tree_insert_b(tree_find_gte_int(tree, ikey, &fnd), k, val);
}
...
Tree tree_insert_dbl(Tree tree, double dkey, Value val) {
    ...
    k.d = dkey;
    return tree_insert_b(tree_find_gte_dbl(tree, dkey, &fnd), k, val);
}

Tree tree_insert_gen(Tree tree, Value key, Value val,
        int (*func)(Value, Value)) {
    int fnd;

    return tree_insert_b(tree_find_gte_gen(tree, key, func, &fnd), key, val);
}
